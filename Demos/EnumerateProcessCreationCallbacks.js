...
class ProcessCallback {
    constructor(index, addr){
        this.index = index;
        this.address = addr;

        // 0x00 PushLock
        // 0x08 / 1 CallbackAddress
        // 0x10 / 2 Flags
        let arr = host.memory.readMemoryValues(addr, 3, 8);
        this.cb = arr[1];
    }

    get symbol(){
        let s = host.getModuleContainingSymbolInformation(this.cb);
        if (!s) return "";
        return s.name;
    }

    toString() {
        return `ProcessCallback(index=${this.index}, address=${this.address}, callback=${this.cb}, symbol=${this.symbol})`;
    }
}

function* GetProcessCallbacks(){  
    let base = host.getModuleSymbolAddress("nt", "PspCreateProcessNotifyRoutine");

    for(let idx = 0; idx <= 0x40 ; idx++ ){
        let ptr = base.add(8 * idx);
        let addr = read64(ptr).bitwiseAnd(-16);
        if (addr.compareTo(0) == 0)
            break;
        yield new ProcessCallback(idx, addr);
    } 
}
...
function initializeScript() {
    return [
        ...
        new host.functionAlias(GetProcessCallbacks, "ProcessCallbacks"),
    ];
}
